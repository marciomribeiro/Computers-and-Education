...
%Even though the strategy pointed students as potential failing ones in the first 30 days, some of them passed, raising false positives. 
...
%We also report that k-means 

%Notice that Figure~\ref{fig:2-2011-01} contains an outlier. In this particular case, the strategy pointed that all students but one would fail the course. Clearly this result is a consequence of such outlier. This way, the presence of one outlier might represent a problem when considering two groups.

%Differently from two groups, here one outlier does not completely compromise the strategy.

%However, notice that applying the strategy with two groups is more likely to outliers problems, such as the one we depict in Figure~\ref{fig:2-2011-01}.